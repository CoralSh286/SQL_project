...
שלב 1: ניתחנו את המבנה הכללי של ה־DSD
...
שלב 2: זיהינו את הישויות (Entities)
...
שלב 3: זיהינו את הקשרים בין הישויות (Relationships)
...
שלב 4: ניתחנו טבלאות קשר (אם היו)
...
ושם פרטי ושם משפחה של מנהל הפרויקט.

באמצעות חיבורים (JOIN) בין טבלאות כמו Volunteer, VolunteerType, Project, Manager, Trained ו־Training, המבט מאפשר לקבל בקלות תצוגה מקיפה של הקשרים בין המתנדב, ההכשרות שקיבל, והפרויקטים בהם הוא מעורב, כולל מידע על המנהל האחראי.
...
כך ניתן לראות איזו הכשרה הייתה הפופולרית ביותר או הנפוצה ביותר בקרב המתנדבים.
 ...
 שאילתה זו מציגה את שמות המתנדבים והפרויקטים בהם הם משתתפים, עבור מתנדבים שעברו לפחות שתי הכשרות שונות.
 
 היא משתמשת במבט volunteer_project_training_view ומבצעת סינון כך שיוצגו רק מתנדבים שמספר ההכשרות שעברו (לפי טבלת Trained) הוא 2 או יותר. 
 
התוצאה כוללת רשומות ייחודיות (בעזרת DISTINCT) כדי להימנע מחזרות, וממוינת לפי שם המשפחה של המתנדב.
